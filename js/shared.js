 // ====== CONEXÃO ======
       ...
       window.onload = function() { 
            ...
            const primeiroDia = `${anoAtual}-01-01`;  // fixa 1º de janeiro do ano atual
            ...
            // Terceirizados (novos campos de data)
            const tercDataInicio = document.getElementById('terc-filter-data-inicio');
            const tercDataFim = document.getElementById('terc-filter-data-fim');
            if(tercDataInicio) tercDataInicio.value = primeiroDia;
            if(tercDataFim) tercDataFim.value = ultimoDiaStr;
        
            // Remove as linhas antigas de mês/ano (se ainda existirem)
    ...
    try {
        const [
          ...
          resEqp, resFases, resPonto, resTerc,
          resPontoDiario, resHistPrecos, resProdTerc
        ] = await Promise.all([
          ...
          fetchAllRecords('jsp_historico_precos').catch(() => ({ data: [] })),
        ...
        STATE.historico_precos = resHistPrecos.data || [];
        STATE.producao_terc = resProdTerc.data || [];
        //console.log("Histórico de Preços carregado:", STATE.historico_precos);
        // Garante que data_preco seja string 'yyyy-mm-dd'
    ...
    const html = `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; width: 100%; border: 2px solid #1e293b; padding: 30px; border-radius: 8px;">
            ...
            <div style="font-size: 14px; line-height: 1.8; text-align: justify; margin-bottom: 40px;">
                ...
                referente ao pagamento de diárias trabalhadas no período de <strong>${periodo}</strong>, totalizando <strong>${totalDiarias} dias</strong> 
...
function executarImpressaoFolha() {
    ...
    filtrados.forEach(c => {
        ...
        if (c.tipo === 'diaria') {
            ...
            const metros = statusFinanceiro === 'PAGO' 
                ? producao.filter(p => p.status === 'PAGO').reduce((acc, p) => acc + parseFloat(p.metros), 0)
    ...
    setTimeout(() => {
        ...
        async function saveObra(e) {
            ...
            // Pegamos o elemento da data com cuidado para não quebrar o código
            ...
            const payload = {
                ...
                data_termino: valorData // Só envia se o campo existir
            };
        
            if(!isNew) payload.id = document.getElementById('obra-id').value;
        
            // Usei "sb" porque foi o que veio no seu exemplo, confirme se é esse mesmo o nome da sua variável do Supabase
    ...
    if(id) {
        ...
        // NOVA LINHA: Carrega a data que vem do banco
        ...
        // Garante que o campo de data comece vazio no novo cadastro
        ...
        function openPosFornecedorModal() { document.getElementById('pos-fornecedor-modal').classList.remove('hidden'); renderPosFornecedorModal(); }
        function closePosFornecedorModal() { document.getElementById('pos-fornecedor-modal').classList.add('hidden'); }
        ...
        function renderOCList() {
           ...
           let arr = Object.values(ocs).sort((a,b)=> parseInt(b.id) - parseInt(a.id));
            ...
            if(arr.length === 0) { list.innerHTML = `<tr><td colspan="6" class="p-8 text-center text-slate-400">Nenhuma Ordem de Compra encontrada.</td></tr>`; return; }
        ...
        async function executeConfirmOC() {
            ...
            await loadData();
            await registrarPrecosAutomaticos(id);
        }
        ...
        async function baixarLog(uid) {
            ...
            if(error) { showLoading(false); return showToast("Erro: "+error.message, true); }
            showToast("Baixa realizada! Valor computado."); loadData();
        }
        ...
        async function estornarBaixa(uid) {
            ...
            if(error) { showLoading(false); return showToast("Erro: "+error.message, true); }
            showToast("Baixa estornada com sucesso!"); loadData();
        }
        ...
        async function excluirLancamento(uid) {
            ...
            if(error) { showLoading(false); return showToast("Erro: "+error.message, true); }
            showToast("Lançamento Cancelado!"); loadData();
        }
        ...
        async function baixarLogOC(id) {
            ...
            if(error) { showLoading(false); return showToast("Erro: "+error.message, true); }
            showToast("O.C. Paga com sucesso!"); loadData();
        }
        ...
        async function estornarBaixaOC(id) {
            ...
            if(error) { showLoading(false); return showToast("Erro: "+error.message, true); }
            showToast("Baixa da O.C. estornada!"); loadData();
        }
        ...
        async function excluirLancamentoOC(id) {
            ...
            if(error) { showLoading(false); return showToast("Erro: "+error.message, true); }
            showToast("O.C. Cancelada no Financeiro!"); loadData();
        }
        ...
        async function saveExpense() {
            ...
            showToast("Despesa Salva!"); loadData();
        }
        ...
        async function saveRevenue() {
            ...
            showToast("Medição Salva!"); loadData();
        }
      ...
      async function estornarPagamentoPonto(ponto_id, equipe_id) {
            ...
            // 1. Volta o status do Ponto para PENDENTE
            ...
            // 2. Busca o lançamento gerado no Financeiro para cancelar e não furar o caixa
            ...
            if(logVinculado) {
                // Se encontrar a despesa, cancela ela
  ...
  // Obtém os mesmos filtros da tela
      ...
      fil.forEach(c => {
        ...
        if (c.tipo === 'diaria') {
          ...
          // Para checagem, normalmente interessa o total produzido (pendente + pago)
          const metrosPendentes = producao.filter(p => p.status !== 'PAGO').reduce((acc, p) => acc + parseFloat(p.metros), 0);
          const metrosPagos = producao.filter(p => p.status === 'PAGO').reduce((acc, p) => acc + parseFloat(p.metros), 0);
          quantidade = metrosPendentes + metrosPagos;
        }
        if (quantidade > 0 || statusFiltro !== 'true') { // mostra mesmo se zero, mas filtro de ativos pode esconder
      ...
      // Monta o HTML de impressão no mesmo padrão profissional
      const hoje = new Date().toLocaleDateString('pt-BR');
      const dataIniFormatada = dataInicio ? new Date(dataInicio + 'T00:00:00').toLocaleDateString('pt-BR') : 'Início';
      const dataFimFormatada = dataFim ? new Date(dataFim + 'T00:00:00').toLocaleDateString('pt-BR') : 'Fim';
